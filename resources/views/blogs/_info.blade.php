  ...
  <div class="panel-body  topic-author-box blog-info">
    ...
    @if ($currentUser && $currentUser->id != $user->id)
      ...
      @if ($currentUser->subscribe($blog))
        <a data-method="post" class="btn btn-default btn-block" href="javascript:void(0);"
           data-url="{{ route('blogs.unsubscribe', $blog->id) }}">
          <i class="fa fa-minus"></i> 取消订阅
        </a>
      @else
        <div class="follow-box">
          <a data-method="post" class="btn btn-primary btn-block" href="javascript:void(0);"
             data-url="{{ route('blogs.subscribe', $blog->id) }}">
