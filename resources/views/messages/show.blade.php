    ...
    <div class="col-md-9  left-col ">
      ...
      <div class="panel panel-default padding-sm">
        ...
        <div class="panel-body">
          <div><a href="{{ route('messages.index') }}" class="normalize-link-color"><i class="fa fa-arrow-left"
          ...
          <form class="form-horizontal" method="POST" action="{{ route('messages.store') }}" accept-charset="UTF-8">
            ...
            <button type="submit" class="btn btn-primary"><i class="fa fa-paper-plane" aria-hidden="true"></i> 发送
            </button>
          ...
          <ul class="list-group row">
            @foreach($messages as $index => $message)
              <li
                class="list-group-item media {{ ($unread_message_count > 0 && $index < $unread_message_count) ? 'unread' : ''  }}"
                style="margin-top: 0px;">
                <div class="avatar pull-left">
                  <a href="{{ route('users.show', [$message->user->id]) }}">
                    <img class="media-object img-thumbnail avatar" alt="{{ $message->user->name }}"
                         src="{{ $message->user->present()->gravatar }}" style="width:48px;height:48px;"/>
                  </a>
                </div>
