        ...
        <div class="panel-body">
          ...
          <div>
            <a href="{{ route('users.show', [$recipient->id]) }}" title="{{ $recipient->name }}">
              <img class="avatar avatar-small" alt="{{ $recipient->name }}"
                   src="{{ $recipient->present()->gravatar }}"/>
              {{ $recipient->name }}
            </a>
          </div>
          <br>
          ...
          <form class="form-horizontal" method="POST" action="{{ route('messages.store') }}" accept-charset="UTF-8">
            ...
            <button type="submit" class="btn btn-primary loading-on-clicked"><i class="fa fa-paper-plane"
