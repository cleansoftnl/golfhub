@if ($currentUser && $currentUser->id == $topic->user->id)
  <a class="popover-with-html text-lg animated rubberBand edit-btn" href="{{ route('users.edit', $topic->user->id) }}"
     data-content="{{ lang('Edit Profile') }}">
    <i class="fa fa-cog"></i>
  </a>
@endif

<a href="{{ route('users.show', $topic->user->id) }}">
  <img src="{{ $topic->user->present()->gravatar }}" style="width:80px; height:80px;margin:5px;"
...
<div class="media-body padding-top-sm">
  @if($topic->user->introduction)
    <div class="media-heading">
      ...
      @if ($topic->user->gender == 'male')
        ...
        <svg width="14" height="16" viewBox="0 0 14 14" class="Icon Icon--male" aria-hidden="true"
             style="height: 16px; width: 14px;fill:#9fadc7;margin-bottom: 3px"><title></title>
          <g>
            <path
      ...
      @elseif($topic->user->gender == 'female')
        ...
        <svg width="14" height="16" viewBox="0 0 12 16" class="Icon Icon--female" aria-hidden="true"
             style="height: 16px; width: 14px;fill: #e89090; margin-bottom: 3px"><title></title>
          <g>
            <path
      ...
      @if ($topic->user->present()->hasBadge())
        <div class="role-label text-white">
          <a class="label label-success role"
             href="{{ route('roles.show', [$topic->user->present()->badgeID()]) }}">{{ $topic->user->present()->badgeName() }}
            }</a>
        </div>
      @endif
  ...
  <ul class="list-inline">
    ...
    @if ($topic->user->real_name)
      <li class="popover-with-html" data-content="{{ lang('Real Name') }}"><span class="org"><i
            class="fa fa-user"></i> {{ $topic->user->real_name }}</span></li>
    @endif
    ...
    @if ($topic->user->github_name)
      <li>
        <a href="https://github.com/{{ $topic->user->github_name }}" target="_blank" class="popover-with-html"
           data-content="{{ $topic->user->github_name }}">
    ...
    @if ($topic->user->weibo_link)
      <li>
        <a href="{{ $topic->user->weibo_link }}" rel="nofollow" class="weibo" target="_blank"><i
            class="fa fa-weibo"></i>
        </a>
      </li>
    @endif
    ...
    @if ($topic->user->twitter_account)
      <li>
        <a href="https://twitter.com/{{ $topic->user->twitter_account }}"
           data-content="{{ $topic->user->twitter_account }}" rel="nofollow" class="twitter popover-with-html"
    ...
    @if ($topic->user->linkedin)
      <li class="popover-with-html" data-content="点击查看 LinkedIn 个人资料">
        <a href="{{ $topic->user->linkedin }}" rel="nofollow" class="linkedin" target="_blank"><i
    ...
    @if ($topic->user->personal_website)
      <li>
        <a href="http://{{ $topic->user->personal_website }}" data-content="{{ $topic->user->personal_website }}"
           rel="nofollow" target="_blank" class="url popover-with-html">
