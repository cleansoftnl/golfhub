...
@section('content')
  ...
  <div class="hall_of_fames">
    ...
    <div class="box  site-intro rm-link-color">
      Laravel China 名人堂，用于推荐对 Laravel China 社区有突出贡献 of user。<a style="text-decoration: underline;"
        ...
        <div class="card {{ $index % 2 == 0 ? 'add-margin-right' : 'add-margin-left' }}">
          ...
          @if ($currentUser && $currentUser->id == $user->id)
            <a class="pull-right popover-with-html text-lg animated rubberBand edit-btn"
               href="{{ route('users.edit', $user->id) }}" data-content="{{ lang('Edit Profile') }}">
              <i class="fa fa-cog"></i>
            </a>
          @endif
            ...
            <ul class="list-inline">
              ...
              @if ($user->real_name)
                <li class="popover-with-html" data-content="{{ lang('Real Name') }}"><span class="org"><i
                      class="fa fa-user"></i> {{ $user->real_name }}</span></li>
              @endif
              ...
              @if ($user->weibo_link)
                <li class="popover-with-html" data-content="{{ $user->weibo_name }}">
                  <a href="{{ $user->weibo_link }}" rel="nofollow" class="weibo" target="_blank"><i
                      class="fa fa-weibo"></i>
                  </a>
                </li>
              @endif
              ...
              @if ($user->twitter_account)
                <li class="popover-with-html" data-content="{{ $user->twitter_account }}">
                  <a href="https://twitter.com/{{ $user->twitter_account }}" rel="nofollow" class="twitter"
