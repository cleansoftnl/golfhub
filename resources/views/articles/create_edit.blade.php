            ...
            @else
              <form method="POST" action="{{ route('articles.store') }}" accept-charset="UTF-8"
                ...
                <div class="form-group">
                  <input class="form-control" id="article-title" placeholder="{{ lang('Please write down a topic') }}"
                         name="title" type="text" value="{{ old('title') ?: $topic->title }}" required="require">
                </div>
                ...
                <div class="form-group">
                  <textarea required="require" class="form-control" rows="20" style="overflow:hidden" id="reply_content"
                            ...
                            cols="50">{{ old('body') ?: $topic->body_original }}</textarea>
                </div>

                <div class="form-group status-post-submit">
                  <button class="btn btn-primary submit-btn" type="submit" name="subject" value="publish"
                          id="{{ $topic->is_draft == 'yes' ? 'publish-hint' : '' }}">{{ lang('Publish') }}</button>
